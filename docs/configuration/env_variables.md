...
| `VLLM_GDN_EXACT_SOLVE`        | Uses exact row-by-row forward substitution instead of the Neumann iterative solver in the gated delta net chunk pipeline. Exact but significantly slower, because it runs one Python-loop iteration per chunk row. Debug-only toggle for isolating accuracy issues to the solver rather than to other parts of the pipeline. | `0` |
