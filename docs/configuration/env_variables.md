...
| `VLLM_GDN_CKPT_SLOTS`         | Per-group checkpoint-pool depth K for compact-GDN prefix caching (applies only when `VLLM_COMPACT_GDN` and prefix caching are both enabled). `0` auto-sizes K from `VLLM_GDN_CKPT_MEM_FRACTION`; a positive value overrides. K is always capped below the block count so the compact memory win is preserved. | `0` (auto) |
| `VLLM_GDN_CKPT_MEM_FRACTION`  | Fraction of the equivalent non-compact GDN footprint budgeted for the checkpoint pool when `VLLM_GDN_CKPT_SLOTS=0`. Auto-K is roughly `mem_fraction × num_blocks / num_gdn_groups`, floored at the concurrent-request count and capped below the block count. Under-provisioning is safe (an evicted prefix falls back to recompute, never a wrong result), so raise it if you serve many long, distinct sessions and observe low `num_cached_tokens`; lower it to reclaim HBM for the KV budget. Ignored when `VLLM_GDN_CKPT_SLOTS` is set. | `0.1` |
| `VLLM_GDN_PC_HIT_CAP`         | Caps a compact-GDN prefix-cache hit at the last block boundary the bounded checkpoint pool still holds, so a request never resumes from an evicted checkpoint. This is a correctness guard (it also drives the engine-core shadow pool that mirrors worker residency at TP>1). Set to `0` only for debugging. | `true` |
