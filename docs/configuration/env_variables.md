...
| `VLLM_MULTIMODAL_RESOLUTIONS` | Pins explicit raw pixel resolutions (`WxH`, comma-separated, e.g. `1024x768,768x1024`) to warm up for native-resolution vision towers. Each resolution is fed through the model's own processor (e.g. `smart_resize`/`navit_resize`) at warmup time, so the compiled vision-tower graph matches what a real request at that resolution produces -- avoiding a runtime recompile. Combines with the `width`/`height` given via `--limit-mm-per-prompt`. Supported for the `image` and `vision_chunk` (Kimi-K2.5/K2.6) modalities. See [Warm-up](../features/warmup.md#multimodal-warm-up). | `None` |
