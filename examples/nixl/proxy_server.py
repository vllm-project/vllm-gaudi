...
AIOHTTP_TIMEOUT = aiohttp.ClientTimeout(total=None, connect=None, sock_read=None, sock_connect=None)
...
async def D_first_token_generator(
    ...
    try:
        ...
        if callback_owner:
            callback_owner.exception_handler(prefill_instance=None, decode_instance=decode_instance, req_len=req_len)
...
class Proxy:
    ...
    async def post_to_instance(self, request: Request, path: str, json_template: dict):
        ...
        try:
            ...
            return JSONResponse({"error": f"Failed to fetch {url}, reason: {str(e)}"}, status_code=500)
    ...
    def get_total_token_length(self, prompt):
        ...
        if isinstance(prompt, str):
            ...
            elif all(isinstance(p, list) and all(isinstance(x, int) for x in p) for p in prompt):
                # Already tokenized
    ...
    async def create_completion(self, raw_request: Request):
        try:
            ...
            try:
                generator_d = self.forward_request(f"http://{decode_instance}/v1/completions", request, request_id)
            except HTTPException as http_exc:
    ...
    async def create_chat_completion(self, raw_request: Request):
        try:
            ...
            start_time = time.time()
            # prefill stage
            total_length = sum(self.get_total_token_length(msg["content"]) for msg in kv_prepare_request["messages"])
            end_time = time.time()
            log_info_green(
            ...
            return StreamingResponse(content=iter(error_messages), media_type="application/json")
...
def parse_instance_spec(instance_spec):
    ...
    # Handle comma-separated ports (e.g., 8300,8301,8302)
        ...
        for port in ports:
            try:
                ...
                raise ValueError(f"Invalid port '{port}' in specification '{instance_spec}'")
    ...
    # Handle single port
        ...
        try:
            ...
            raise ValueError(f"Invalid port '{port_spec}' in specification '{instance_spec}'")
