...
log_name="test1-online-${model_name}-kakao-profiledevice"

hl-prof-config --use-template profile_api --hw-trace on 
...
until [[ "$n" -ge 1000 ]] || [[ $ready == true ]]; do
    n=$((n+1))
    if grep -q "Token throughput (total)" benchmark_logs/${log_name}_run1.log; then
        break
    fi
    sleep 5s
done
