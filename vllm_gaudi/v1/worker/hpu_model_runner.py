...
class HPUModelRunner(HpuKVConnectorModelRunnerMixin):
    ...
    def _align_and_pad_mrope_positions(
        ...
        out_shape = (3, target_len) if target_bs == 1 else (3, target_bs * target_len)
                    ...
                    if inner_mla_attn is not None:
                        ...
                        for m in duplicate_mods:
                            if hasattr(mla_attn, m) and hasattr(canonical_mla_owner, m):
                                delattr(mla_attn, m)
                            if (
                                inner_mla_attn is not canonical_mla_owner
                                and hasattr(inner_mla_attn, m)
    ...
    def warmup_multimodal_graphs(self, buckets):
        ...
        is_image_warmup = (
            ...
            and mm_config.limit_per_prompt.get("image") is not None
            and "image" in self.mm_budget.mm_limits
            and self.mm_budget.mm_limits["image"] != 0
        )
        is_video_warmup = (
            mm_config is not None
            and mm_config.limit_per_prompt.get("video") is not None
        ...
        for modality, (limit_value, get_options) in warmup_configs.items():
            if (
                mm_config
                and mm_config.limit_per_prompt.get(modality) is not None
                and modality in self.mm_budget.mm_limits
    ...
    def warmup_model(self) -> None:
        ...
        if not self.is_pooling_model and not self.num_mamba_like_layers and max_bucket > self.input_batch.max_num_reqs:
        ...
        if not (self.num_mamba_like_layers or self.is_pooling_model) and max_bucket > self.input_batch.max_num_reqs:
