...
def swap_blocks(
    ...
    if is_hetero:  # Not verified yet
        ...
        if len(src_block_ids) == src_block_ids[-1] - src_block_ids[0] + 1:  # simple check if the indices are contiguous
            ...
            dst_kv_caches[0][block_idx * block_size : (num_blocks + block_idx) * block_size] = (
                key_cache[block_idx * block_size : (num_blocks + block_idx) * block_size]
                .reshape(num_blocks * block_factor, n_kv_heads, remote_block_size, head_dim)
                .permute(0, 2, 1, 3)
                .contiguous()
                .reshape(num_blocks * block_size, n_kv_heads, head_dim)
            )
            dst_kv_caches[1][block_idx * block_size : (num_blocks + block_idx) * block_size] = (
                value_cache[block_idx * block_size : (num_blocks + block_idx) * block_size]
                .reshape(num_blocks * block_factor, n_kv_heads, remote_block_size, head_dim)
                .permute(0, 2, 1, 3)
                .contiguous()
                .reshape(num_blocks * block_size, n_kv_heads, head_dim)
        ...
        for block_idx in src_block_ids:
            dst_kv_caches[0][block_idx * block_size : (1 + block_idx) * block_size] = (
                key_cache[block_idx * block_size : (1 + block_idx) * block_size]
                .reshape(block_factor, n_kv_heads, remote_block_size, head_dim)
                .permute(0, 2, 1, 3)
                .contiguous()
                .reshape(block_size, n_kv_heads, head_dim)
                .to("hpu")
            )
            dst_kv_caches[1][block_idx * block_size : (1 + block_idx) * block_size] = (
                value_cache[block_idx * block_size : (1 + block_idx) * block_size]
                .reshape(block_factor, n_kv_heads, remote_block_size, head_dim)
                .permute(0, 2, 1, 3)
                .contiguous()
                .reshape(block_size, n_kv_heads, head_dim)
...
def transfer_async(self, job_id: int, transfer_spec: TransferSpec) -> bool:
    ...
    with torch.hpu.stream(stream):
        ...
        for src_tensor, dst_tensor, block_size_in_bytes in zip(
            ...
            swap_blocks(
                src_tensor, dst_tensor, src_to_dst_tensor, "d2h" if self.src_tensors[0].device.type == "hpu" else "h2d"
            )
