...
def rejection_sample_pytorch(
    ...
    # cumulative sum
    start_indices = torch.cat((torch.tensor([0],
                                            device=device,
    ...
    num_accepted = ((first_mismatch_idx + 1) * any_mismatch +
                    num_draft_tokens_per_seq * (~any_mismatch))
...
def rejection_sample(
    ...
    output_token_ids = rejection_sample_pytorch(draft_token_ids, target_argmax,
                                                bonus_token_ids,
                                                cu_num_draft_tokens)
