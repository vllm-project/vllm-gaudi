...
class HPUMPLinearKernel(MPLinearKernel):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if c.has_g_idx:
            ...
            if c.group_size > 0:
                g_idx_trivial = [i // c.group_size for i in range(columns)]
            else:
                g_idx_trivial = [0] * columns
            g_idx_trivial = torch.tensor(g_idx_trivial, dtype=torch.int32)
            assert torch.equal(w_gidx, g_idx_trivial.to('hpu')), "Non-trivial tensor g_idx is not supported"
        
        def transform_w_q(x):
            assert isinstance(x, BasevLLMParameter)
            qweight = unpack_quantized_values_into_int32(x.data.transpose(0, 1),
                                                         c.weight_type,
                                                         packed_dim=0)
        ...
        if c.zero_points:
            ...
            if c.weight_type.has_bias():
                ...
                raise NotImplementedError(
                    "A 0 zero-point is not supported on HPU compressed wNa16 format")
...
class HPUCompressedTensorsWNA16MoEMethod(CompressedTensorsWNA16MoEMethod):
    ...
    def create_weights(self, layer: torch.nn.Module, num_experts: int,
        ...
        w2_zeros = torch.full((num_groups_w2, hidden_size),
                              self.quant_type.bias,
                              dtype=torch.int32)
    ...
    def gptq_hpu_moe_repack(self, b_q_weight: torch.Tensor) ->  torch.Tensor:
        ...
        for e in range(num_experts):
            weight = unpack_quantized_values_into_int32(b_q_weight[e].data.contiguous().transpose(0,1), self.quant_type, packed_dim=0)
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        layer.w13_weight_scale = torch.nn.Parameter(layer.w13_weight_scale.data.transpose(1, 2).contiguous(), requires_grad=False)
        layer.w2_weight_scale = torch.nn.Parameter(layer.w2_weight_scale.data.transpose(1, 2).contiguous(), requires_grad=False)
        ...
        for expert_id in range(layer.local_num_experts):
            layer.moe_op.w13_list[expert_id].set_weight_packed(
                layer.w13_weight_packed.data[expert_id])
            layer.moe_op.w2_list[expert_id].set_weight_packed(
                layer.w2_weight_packed.data[expert_id])
            layer.moe_op.w13_list[expert_id].set_weight_scale(
                layer.w13_weight_scale.data[expert_id])
            layer.moe_op.w2_list[expert_id].set_weight_scale(
                layer.w2_weight_scale.data[expert_id])
            layer.moe_op.w13_list[expert_id].set_zero_point(
                layer.w13_zero_point.data)
            layer.moe_op.w2_list[expert_id].set_zero_point(
                layer.w2_zero_point.data)
