...
class HPUQwen3_5GatedDeltaNet(Qwen3_5GatedDeltaNet):
    ...
    def _forward_prefill_phase_b_only(
        ...
        if out_squeezed.shape[0] == core_attn_out.shape[0]:
            core_attn_out.copy_(out_squeezed)
        else:
            n = min(out_squeezed.shape[0], core_attn_out.shape[0])
            core_attn_out[:n] = out_squeezed[:n]

    @torch._dynamo.disable
    def _forward_prefill_from_conv(
        self,
        query: torch.Tensor,
        key: torch.Tensor,
        value: torch.Tensor,
        ...
        """Prefill from after conv1d — chunk recurrence + write-back."""
        # Chunk recurrence
        initial_state = ssm_state[state_indices].contiguous()
        initial_state[~has_initial_state.bool(), ...] = 0

        core_attn_out_result, last_recurrent_state = hpu_chunk_gated_delta_rule(
            q=query,
            k=key,
            v=value,
            g=g,
    ...
    @torch._dynamo.disable
    def _forward_prefill_from_gating(
        self,
        ...
        """Prefill from after gating — conv1d + chunk recurrence + write-back."""
        num_tokens = mixed_qkv.size(0)

        # Build per-sequence padded cu_seqlens for chunk recurrence
        ...
        if state_indices is not None and token_mask_flat is not None:
            ...
            padded_seq_len = num_tokens // num_rows
        ...
        # 2. Rearrange to [1, T, H, D]
        query, key, value = self.rearrange_mixed_qkv(mixed_qkv_conv)

        # 3. Apply token mask to gating
        if token_mask_flat is not None:
            token_mask_h = token_mask_flat.view(1, -1, 1).to(dtype=g.dtype)
            g = g * token_mask_h
            beta = beta * token_mask_h
    ...
    def _forward_prefill_eager(
        ...
        query_start_loc: torch.Tensor,
        has_initial_state: torch.Tensor,
        padding_mask_flat: torch.Tensor | None,
    ) -> None:
        """Prefill path — runs entirely eagerly (has loops, CPU transfers)."""
        num_tokens = mixed_qkv.size(0)
