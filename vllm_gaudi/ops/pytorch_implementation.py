...
def new_chunk_state(B, x, dt, dA_cumsum, states=None, states_in_fp32=True):
    ...
    x_dtype = x.dtype
    B = B.float().view(nchunks, chunk_size, ngroups, 1, dstate).expand(-1, -1, -1, nheads_ngroups_ratio, -1).reshape(nchunks, chunk_size, nheads, dstate)
...
def new_ssd_state_passing(states, dA_cumsum, initial_states=None, out_dtype=None):
    ...
    states_t = initial_states[0].to(dtype=compute_dtype, device=device) if initial_states is not None else torch.zeros((nheads, hdim), device=device, dtype=compute_dtype)
