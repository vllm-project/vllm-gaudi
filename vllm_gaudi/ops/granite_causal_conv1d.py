...
def granite_causal_conv1d_fn(
    ...
    if padded_batch > 1:
        assert not enable_prefix_caching, ("Prefix caching with batched prefill (BS>1) is not yet supported "
                                           "for Granite causal conv1d.")
        ...
        if has_initial_state is not None:
            raw_states = conv_states[load_cache_indices, -state_len:, :]  # [BS, state_len, dim]
        ...
        # store_cache_indices may contain repeated PAD block ids;
        # advanced-index assignment with duplicate indices is undefined
        # in general. To keep the write deterministic, zero the content
        # of padding rows so that every duplicate write stores the same
        # value. Real (non-padding) store_cache_indices are unique per
        # request.
        ...
        conv_states[store_cache_indices, -state_len:, :] = new_states_T
