...
class HPUVocabParallelEmbeddingWithLoRA(VocabParallelEmbeddingWithLoRA):
    ...
    def forward_oot(self, x: torch.Tensor) -> torch.Tensor:
        ...
        self.punica_wrapper.add_lora_embedding(full_output, full_lora_a_embeddings, self.lora_b_stacked, add_input=True)
        # can be removed on moving to flat tensors
        full_output_org = full_output_org.view(shape[0], shape[1], full_output_org.shape[1])
