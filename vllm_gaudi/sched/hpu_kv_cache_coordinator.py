...
from vllm_gaudi.sched.hpu_block_pool import HPUBlockPool
...
class HPUKVCacheCoordinator(KVCacheCoordinator):
    """
    ...
    def __init__(
        ...
        self.block_pool = HPUBlockPool(kv_cache_config.num_blocks, enable_caching, enable_kv_cache_events)
...
class HPUKVCacheCoordinatorNoPrefixCache(HPUKVCacheCoordinator):
    """
...
class HPUUnitaryKVCacheCoordinator(HPUKVCacheCoordinator):
    """
    ...
    def __init__(
        ...
        assert len(
            self.kv_cache_config.kv_cache_groups) == 1, ("UnitaryKVCacheCoordinator assumes only one kv cache group")
...
class HPUHybridKVCacheCoordinator(HPUKVCacheCoordinator):
    """
    ...
    def verify_and_split_kv_cache_groups(self) -> None:
        ...
        for i, g in enumerate(self.kv_cache_config.kv_cache_groups):
            if isinstance(g.kv_cache_spec, FullAttentionSpec):
                ...
                if other_spec is None:
                    ...
                    assert other_spec == g.kv_cache_spec, ("HybridKVCacheCoordinator assumes "
                                                           "exactly one other type of groups now.")
                self.other_group_ids.append(i)

        assert full_attention_spec is not None, ("HybridKVCacheCoordinator assumes exactly one type of full "
        ...
        if self.enable_caching:
            ...
            assert divisible == 0, ("KVCacheCoordinator assumes the block_size of full "
        ...
        if max(self.full_attention_group_ids) < min(self.other_group_ids):
            ...
            raise ValueError("HybridKVCacheCoordinator assumes the full "
...
def get_kv_cache_coordinator(
    ...
    if not enable_caching:
        return HPUKVCacheCoordinatorNoPrefixCache(
            kv_cache_config,
            max_model_len,
            use_eagle,
            enable_kv_cache_events,
    ...
    if len(kv_cache_config.kv_cache_groups) == 1:
        return HPUUnitaryKVCacheCoordinator(
            kv_cache_config,
            max_model_len,
            use_eagle,
            enable_caching,
    ...
    return HPUHybridKVCacheCoordinator(
        kv_cache_config,
        max_model_len,
        use_eagle,
        enable_caching,
