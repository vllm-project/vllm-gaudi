...
def calc_USABLE_MEM(ctx):
    return (
        (ctx["TOTAL_GPU_MEM"] / ctx["TENSOR_PARALLEL_SIZE"])
...
def calc_EST_MAX_NUM_SEQS(ctx):
    return max(
        1, ctx["TENSOR_PARALLEL_SIZE"] * ctx["USABLE_MEM"] * ctx["GPU_MEM_UTILIZATION"] / ctx["KV_CACHE_PER_SEQ"]
    )
...
def calc_PROMPT_BS_STEP_GRAPHS(ctx):
    if ctx["VLLM_EXPONENTIAL_BUCKETING"]:
        ...
        return max(
            ...
            int(
                1
                + (ctx["VLLM_PROMPT_BS_BUCKET_MAX"] - ctx["VLLM_PROMPT_BS_BUCKET_STEP"])
...
def calc_EST_PROMPT_GRAPH_MEM(ctx):
    if ctx["VLLM_EXPONENTIAL_BUCKETING"]:
        ...
        est_prompt_graph_mem = (
            ctx["EST_NUM_PROMPT_GRAPHS"]
            * ctx["APPROX_MEM_PER_GRAPH_MB"]
            * pow(max(1, ctx["MAX_MODEL_LEN"] / 4352), 0.8552)
        )
...
def calc_EST_GRAPH_RESERVE_MEM(ctx):
    return (
        math.ceil(
            ctx["DECODE_GRAPH_TARGET_GB"]
            / (ctx["USABLE_MEM"] * ctx["GPU_MEM_UTILIZATION"] * (1 - ctx["EST_GRAPH_PROMPT_RATIO"]))
            * 100
        )
        / 100
    )
...
def calc_MAX_NUM_SEQS(ctx):
    ...
    if (
        ctx["MODEL"] in ["meta-llama/Llama-3.2-11B-Vision-Instruct", "meta-llama/Llama-3.2-90B-Vision-Instruct"]
        and val > 128
    ):
        print(f"{ctx['MODEL']} currently does not support max-num-seqs > 128. Limiting max-num-seqs to 128")
        val = 128
    if val < 1:
        raise ValueError(
