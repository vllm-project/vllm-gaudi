# Qwen/Qwen3-VL-32B-Thinking for interactive with batch size 16 with SLA TTFT <2s 
...
vllm serve \
  ...
  --trust-remote-code 2>&1 | tee Qwen3-VL-32B-Thinking_server.txt

#benchamark command 
vllm bench serve \
  --backend openai-chat \
  --model Qwen/Qwen3-VL-32B-Instruct-Thinking \
  --tokenizer Qwen/Qwen3-VL-32B-Thinking \
  ...
  --trust-remote-code 2>&1 | tee Qwen3-VL-32B-Thinking_benchmark.txt
