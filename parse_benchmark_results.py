...
def parse_summary_section(filepath):
    ...
    summary_sections = re.findall(
        ...
        r'-+\n(.*?)'
        r'>>> Recommended cutoff for num_chunks=\d+: (\d+) tokens',
        content,
        re.DOTALL
    )
...
def main():
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        if not df_summaries.empty:
            pivot_speedup = df_summaries.pivot_table(
                values='Speedup',
                index=['Tokens'],
                columns=['TP Size', 'Num Chunks'],
                aggfunc='first'
            ...
            cutoff_summary = df_summaries.groupby(['TP Size', 'Num Chunks']).agg({
                'Cutoff': 'first'
            }).reset_index()
