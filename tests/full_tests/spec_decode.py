...
def test_eagle_model(is_enable, args, prompts, sampling_params, task_key, result_queue):
    VLLM_CLS = LLM if prompts is not None else VLLM
    kwargs = {"model":"/mnt/weka/data/pytorch/llama3/Meta-Llama-3-8B-Instruct"} if prompts is not None \
        else {"pretrained":"/mnt/weka/data/pytorch/llama3/Meta-Llama-3-8B-Instruct","batch_size":"16"}
    try:
        if not is_enable:
            llm = VLLM_CLS(
                **kwargs,
