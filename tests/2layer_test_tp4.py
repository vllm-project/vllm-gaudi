...
def test_llama_70b_2layer_tp4_bf16(
    max_tokens: int = 16, warmup: bool = True, prompt_len: int = 8192, profile: bool = False, num_decodes: int = 1
):
    """
    ...
    try:
        ...
        llm = LLM(
            ...
            num_gpu_blocks_override=((num_decodes * max_tokens_per_seq)//128 + 10),  # Slightly larger than max decode to avoid recompilations. This is used because defrag=True fails so we avoid defragmentation limiting kv cache
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-decodes", type=int, default=1, help="Batch size for decode (number of concurrent requests)")
    args = parser.parse_args()
    
    test_llama_70b_2layer_tp4_bf16(
        max_tokens=args.max_tokens, warmup=not args.no_warmup, prompt_len=args.prompt_len, 
        profile=args.profile, num_decodes=args.num_decodes
    )
