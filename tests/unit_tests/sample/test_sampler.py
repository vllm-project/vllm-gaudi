...
from vllm_gaudi.utils import make_tensor_with_pad
from vllm.v1.sample.logits_processor import LogitsProcessorManager
from vllm.v1.sample.metadata import SamplingMetadata
from vllm.v1.sample.sampler import Sampler

LOGITS_DTYPES = [torch.float, torch.bfloat16, torch.float16]
MAX_NUM_REQS = 256
VOCAB_SIZE = 1024
...
def _create_fake_logits(batch_size: int, vocab_size: int,
                        dtype: torch.dtype) -> torch.Tensor:
    fake_logits = torch.full((batch_size, vocab_size), 1e-2, dtype=dtype)
    return fake_logits
...
@pytest.mark.parametrize("logits_dtype", LOGITS_DTYPES)
...
def test_sampler_presence_penalty(device: str, batch_size: int,
    ...
    # logit value.
    fake_logits = _create_fake_logits(batch_size, VOCAB_SIZE, logits_dtype)
