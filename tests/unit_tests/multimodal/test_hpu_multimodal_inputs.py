...
@pytest.mark.parametrize("batch_size,tensor_shape", [
...
@pytest.mark.parametrize("tensor_shapes", [
...
def test_hpu_nested_multimodal_batch():
    ...
    result = MultiModalKwargs.batch(batch_data)
    expected = {"image":torch.stack([a.unsqueeze(0),
                            b.unsqueeze(0)])}
...
@pytest.mark.parametrize("tensor_dtypes", [
    ...
    pytest.param((torch.bfloat16, torch.float32), marks=pytest.mark.xfail(reason="Mixed precision (bf16+fp32) not supported on HPU"))
])
...
@pytest.mark.parametrize("tensor_size,batch_count", [
    ([2, 2], 3),           # Small tensors
...
def test_hpu_multimodal_kwargs_keys():
    ...
    batch_data = [
        {"image": image_tensor, "audio": audio_tensor}
    ]
