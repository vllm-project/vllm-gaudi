...
def test_stop_by_max_tokens(max_tokens: int, oot_scheduler: bool):
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_abort(oot_scheduler: bool):
    scheduler = create_scheduler(async_scheduling=True,
                                 oot_scheduler=oot_scheduler)
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_preempt(oot_scheduler: bool):
    scheduler = create_scheduler(async_scheduling=True,
                                 oot_scheduler=oot_scheduler)
    requests = create_requests(num_requests=10, max_tokens=20)
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_prefix_caching_for_prefill_dedup(oot_scheduler: bool):
    CHUNK_SIZE = 1000
    BLOCK_SIZE = 16
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_prefix_caching_for_multi_turn(oot_scheduler: bool):
    CHUNK_SIZE = 1000
    BLOCK_SIZE = 16
    num_prompt_tokens = 100
