...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_add_requests(oot_scheduler: bool):
    scheduler = create_scheduler(oot_scheduler=oot_scheduler)
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_finish_request(oot_scheduler: bool):
    scheduler = create_scheduler(oot_scheduler=oot_scheduler)
    requests = create_requests(num_requests=10)
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_get_num_unfinished_requests(oot_scheduler: bool):
    scheduler = create_scheduler(oot_scheduler=oot_scheduler)
    requests = create_requests(num_requests=10)
    for request in requests:
        scheduler.add_request(request)
...
def test_schedule(enable_prefix_caching: Optional[bool],
                  prompt_logprobs: Optional[int], oot_scheduler: bool):
    '''Test scheduling.
    Two cases: default APC/no prompt logprobs; APC=True + prompt logprobs
    '''
    scheduler = create_scheduler(enable_prefix_caching=enable_prefix_caching,
                                 oot_scheduler=oot_scheduler)
    requests = create_requests(num_requests=10,
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_schedule_multimodal_requests(oot_scheduler: bool):
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_schedule_partial_requests(oot_scheduler: bool):
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_no_mm_input_chunking(oot_scheduler: bool):
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_stop_via_update_from_output(oot_scheduler: bool):
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_preempt_during_execution(oot_scheduler: bool):
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_kv_connector_basic(oot_scheduler: bool):
    """
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_kv_connector_unable_to_allocate(oot_scheduler: bool):
    """
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_kv_connector_handles_preemption(oot_scheduler: bool):
    """
    Test whether scheduler with KVConnector is able to handle
    unable to allocate (run out of blocks in allocate_slots().
    """
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_memory_leak(oot_scheduler: bool):
...
def create_scheduler_with_priority(
    ...
    # Determine scheduler module and class based on configuration
    scheduler_map = {
        (True, False): ("vllm_gaudi.sched.hpu_scheduler", "HPUScheduler"),
        (True, True): ("vllm_gaudi.sched.hpu_scheduler", "AsyncHPUScheduler"),
        (False, False): ("vllm.v1.core.sched.scheduler", "Scheduler"),
        (False, True):
        ("vllm.v1.core.sched.async_scheduler", "AsyncScheduler"),
    }

    # For priority scheduling, we don't use async scheduling
    async_scheduling = False
    module_name, class_name = scheduler_map[(oot_scheduler, async_scheduling)]
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_priority_scheduling_basic_ordering(oot_scheduler: bool):
    """Test that requests are scheduled in priority order
    (lower value = higher priority)."""
    scheduler = create_scheduler_with_priority(oot_scheduler=oot_scheduler)
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_priority_scheduling_arrival_time_tiebreaker(oot_scheduler: bool):
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_priority_scheduling_mixed_priority_and_arrival(oot_scheduler: bool):
...
@pytest.mark.parametrize("oot_scheduler", [True, False])
def test_priority_scheduling_preemption(oot_scheduler: bool):
