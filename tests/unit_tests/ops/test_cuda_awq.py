...
def test_gptq_linear_method():
    config = {"bits": 4,
              "group_size": 128,
              "zero_point": True}
    native_quant_config = AWQConfig.from_config(config)
    ...
    assert isinstance(native_op.quant_method, AWQLinearMethod)

    import pdb
    pdb.set_trace()

    qweight = torch.load("/software/users/kpietkun/awq/cuda_qweight.pt", weights_only=False, map_location="cuda")
    native_op.qweight.copy_(qweight)
    qzeros = torch.load("/software/users/kpietkun/awq/cuda_qzeros.pt", weights_only=False, map_location="cuda")
    native_op.qzeros.data.copy_(qzeros)
    scales = torch.load("/software/users/kpietkun/awq/cuda_scales.pt", weights_only=False, map_location="cuda")
    ...
    input = torch.load("/software/users/kpietkun/awq/cuda_input.pt", weights_only=False, map_location="cuda")
    ref_output = torch.load("/software/users/kpietkun/awq/cuda_output.pt", weights_only=False, map_location="cuda")
